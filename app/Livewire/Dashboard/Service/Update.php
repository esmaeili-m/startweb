...
class Update extends Component
{
    ...
    public function rules()
    {
        return [
            'title' => 'required',
            'slug' => 'required|unique:services,slug,' . $this->item->id,
        ];
    }
    protected $messages=[
    ...
    {
        $this->item=Services::with('tags')->find($id);
        $this->fill([
    ...
    public function save()
    {
        ...
        TagsRelations::where('type','service')->where('relation_id',$this->item->id)->delete();
        ...
        session()->flash('message','خدمات با موفقیت آپدیت شد');
    ...
    {
        $this->image=upload_file($this->image,'services');
    }
